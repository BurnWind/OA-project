            ...
            try:
                ...
                if check_password(password, user_obj.user_passwd):
                    ...
                    if Role.objects.get(pk=user_obj.role_id).role_name == '超级管理员':
                        ...
                        return response
                    elif Role.objects.get(pk=user_obj.role_id).role_name == '部门主管':
                        response = render(
                            request, 'superior-homepage.html', {'notice': notice, 'message': message, 'staff_name': staff_name})
                        return response
                    elif Role.objects.get(pk=user_obj.role_id).role_name == '普通员工':
    ...
    if request.method == 'POST':
        ...
        if app == 'pass':
            ...
            for i in ids:
                ...
                out.statu = 'refuse'
                Message.objects.create(staff_id=staff.staff_id, title='外出审批回复',
                                       content='您从{}到{}的外出已被拒绝!!'.format(out.startday, out.endday))
            return HttpResponse('操作成功' + exmess)
    return HttpResponseNotFound('404')
